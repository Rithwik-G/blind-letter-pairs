...
export default function Account({ session }) {
  ...
  async function saveCsv() {
    // Convert 2D data array to CSV text with Papa.unparse
    // Optionally add headers (A-Z) for columns and rows if you want
    const csvText = Papa.unparse(data)

    const { error } = await supabase.from('user_csv_data').insert([
      {
        id: userId,
        filename,
        csv_content: csvText,
